...
class LessonSequencer:
    ...
    def _run_doc_mode(self, stdscr):
        # Read-only viewer with n/p navigation, and 'r' for rote mode
        curses.curs_set(0)
        stdscr.nodelay(True)
            ...
            while True:
                try:
                    ...
                    if key == 3:  # Ctrl+C
                        ...
                        rote_completed = self._run_rote_mode(stdscr, self.lessons[idx])
                        if rote_completed:
                            # Finished 10 reps, show boom and return to doc_mode
                            self._display_boom(stdscr)
    ...
    def _run_rote_mode(self, stdscr, lesson):
        ...
        while reps_completed < ROTE_TARGET:
            ...
            while rep_in_progress:
                if need_redraw:
                    ...
                    title = f"Rote Mode: {self.name} | {lesson.name} | Rep {reps_completed}/{ROTE_TARGET}"
                    try:
                        stdscr.addstr(0, 0, title, curses.color_pair(1))
                        stdscr.move(0, len(title))
                        stdscr.clrtoeol()
    ...
    def _display_boom(self, stdscr):
        ...
        try:
            stdscr.addstr(
                max_y - 1,
                0,
                "Rote complete! Press any key to return.",
                curses.color_pair(1),
            )
        except curses.error:
            pass
